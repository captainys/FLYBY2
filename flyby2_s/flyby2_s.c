...
{
	int ac,cmd;
	char *av[16],str[128],ful[256];
	FILE *fp;

	int firstWarning;

	firstWarning=BI_OFF;
	fp=fopen("c:\\windows\\flyby2_s.inf","r");
	if(fp!=NULL)
	{
		nAir=0;
		while(fgets(str,128,fp)!=NULL)
		{
			...
			if(ac>0)
			{
				...
				case 0:
					...
					/* Important : Use ArLoad*** Instead of BiLoad*** */
					else if(ArLoadSrf(&air[nAir],ful)==BI_OK)
					{
						nAir++;
					}
					else
