...
int BiEpsOpen(char *fn,int wid,int hei)
{
	...
	if(BiEpsMode==BI_OFF)
	{
		...
		ep=BiBeginEps(fn,A4LEFT,A4BOTTOM,wid,hei,wid,hei);
		if(ep!=NULL)
		{
			eWid=wid;
			eHei=hei;
...
int BiEpsResetProjection(BIPROJ *prj)
{
	...
	/* I confess that I failed to design this part.
...
int BiEpsDrawEllipse(BIPOINTS *c,long radx,long rady,BICOLOR *col)
{
	...
	if(BiEpsMode==BI_ON)
	{
		p=BiPushTmpStack(&stk,sizeof(BIPOINTS)*32);
		if(p!=NULL)
		{
			for(i=0; i<32; i++)
			{
...
int BiEpsDrawCircle3(BIPOINT *p,real rad,BICOLOR *col)
{
	if(BiEpsMode==BI_ON)
	{
		...
		BiEpsDrawEllipse(&s,sradx,srady,col);
		return BI_OK;
	}
	return BI_ERR;
}
