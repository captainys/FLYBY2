...
/* depend */void BiOpenWindow(long wx,long wy)
{
	...
	htnPlt=CreateHalftonePalette(BiWndDc);
	oldPlt=SelectPalette(BiWndDc,htnPlt,FALSE);
...
static void BiGenerateDrawBuffer(void)
{
	...
	dBuf=CreateCompatibleDC(BiWndDc);
	bmp=CreateCompatibleBitmap(BiWndDc,winX,winY);
	oldBmp=SelectObject(dBuf,bmp);

	SetBkMode(dBuf,TRANSPARENT);
	SetPolyFillMode(dBuf,ALTERNATE);
}
...
static void BiResizeDrawBuffer(int w,int h)
{
	...
	winX=w;
	winY=h;
	bmp=CreateCompatibleBitmap(BiWndDc,w,h);
	bmp=SelectObject(dBuf,bmp);
	DeleteObject(bmp);
}
...
{
	BitBlt(BiWndDc,0,0,winX,winY,dBuf,0,0,SRCCOPY);
}
...
static void BiSetLineColor(BICOLOR *ptr)
{
	...
	orgPen=SelectObject(dBuf,neoPen);
	orgBrs=SelectObject(dBuf,GetStockObject(NULL_BRUSH));
}

static void BiEndLineColor(void)
{
	SelectObject(dBuf,orgPen);
	SelectObject(dBuf,orgBrs);
	DeleteObject(neoPen);
}

static void BiSetRegionColor(BICOLOR *ptr)
{
	neoBrs=CreateSolidBrush(COLOR(ptr));
	orgBrs=SelectObject(dBuf,neoBrs);
	orgPen=SelectObject(dBuf,GetStockObject(NULL_PEN));
}
...
/* depend */void BiClearScreenD(void)
{
	...
	BIPOINTS s1,s2;
	BiSetPointS(&s1,0,0);
	BiSetPointS(&s2,winX-1,winY-1);
	BiDrawRect2D(&s1,&s2,&col);
}
...
/* depend */void BiDrawLine2D(BIPOINTS *s1,BIPOINTS *s2,BICOLOR *c)
{
	...
	{
		MoveToEx(dBuf,s1->x,s1->y,NULL);
		LineTo(dBuf,s2->x,s2->y);
	}
	else if(BiClipLineScrn(&a1,&a2,s1,s2)==BI_IN)
	{
		MoveToEx(dBuf,a1.x,a1.y,NULL);
		LineTo(dBuf,a2.x,a2.y);
 	}
	BiEndLineColor();
...
/* depend */void BiDrawPset2D(BIPOINTS *s,BICOLOR *c)
{
	...
	{
		SetPixel(&dBuf,t1.x,t1.y,COLOR(c));
		SetPixel(&dBuf,t2.x,t1.y,COLOR(c));
		SetPixel(&dBuf,t1.x,t2.y,COLOR(c));
		SetPixel(&dBuf,t2.x,t2.y,COLOR(c));
	}
}
...
/* depend */void BiDrawCircle2D(BIPOINTS *s,long radx,long rady,BICOLOR *c)
{
	...
	{
		BiSetRegionColor(c);
		Ellipse(dBuf,s->x-radx,s->y-rady,s->x+radx,s->y+rady);
		BiEndRegionColor();
	}
}
...
CLIPPING:
	...
	{
		BiSetRegionColor(c);
		Polygon(dBuf,(CONST POINT *)sNew,nsNew);
		BiEndRegionColor();
	}
	BiWinResetClipScrnRect();
	BiPopTmpStack(stk);
...
/* depend */void BiDrawRect2D(BIPOINTS *s1,BIPOINTS *s2,BICOLOR *c)
{
	...
	{
		BiSetRegionColor(c);
		Rectangle(dBuf,s1->x,s1->y,s2->x+2,s2->y+2);/* Why +2? I don't know.*/
		BiEndRegionColor();
	}
	else
	{
		...
		BiSetRegionColor(c);
		Rectangle(dBuf,a1.x,a1.y,a2.x+2,a2.y+2);/* Why +2? I don't know.*/
		BiEndRegionColor();
	}
}
...
/* depend */void BiDrawString2D(BIPOINTS *s,char *str,BICOLOR *c)
{
	...
	{
		HFONT stk;
		stk=SelectObject(dBuf,GetStockObject(SYSTEM_FIXED_FONT));
		SetTextColor(dBuf,COLOR(c));
		TextOut(dBuf,(int)s->x,(int)s->y-16,(LPCSTR)str,strlen(str));
		SelectObject(dBuf,stk);
	}
}
