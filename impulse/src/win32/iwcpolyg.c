	...
	(unsigned char *bmp,int x1,int x2,register unsigned long pat)
{
	...
	if(x1&1)
	{
		bmp[x1]=(unsigned char)(pat&255);
		x1++;
	}
	if(x1+1<=x2)
	{
		*((unsigned short *)(bmp+x1))=(unsigned short)(pat&65535);
	}
	x1=(x1+3)&~3;
	...
	if(x1<=x2)
	{
		...
		{
		case 0:
			bmp[x2]=(unsigned char)(pat&255);
			break;
		case 1:
			*((unsigned short *)(bmp+x2))=(unsigned short)(pat&65535);
			break;
		case 2:
			*((unsigned short *)(bmp+x2))=(unsigned short)(pat&65535);
			bmp[x2+2]=(unsigned char)(pat&255);
			break;
		case 3:
			*((unsigned long *)(bmp+x2))=pat;
			break;
		}
	}
	*/
