...
/***********************************************************************
void WcGetColorIndex(int *idx,WCCOLOR *col);
***********************************************************************/
...
static int WcCreateDC(WCBMP *bm,int wid,int hei);
...
int WcCreateBmp(WCBMP *bm,int wid,int hei)
{
	bm->alive=0;
	WcAdjustWidth(&wid);
	if(WcCreateDC(bm,wid,hei)==WC_OK)
	{
		bm->alive=1;
		return 0;
	}
...
static int WcCreateBitmap(WCBMP *bm,int wid,int hei)
{
	...
	bm->bmp=WinGCreateBitmap(bm->dc,(BITMAPINFO *)&inf,&bit);
	if(bm->bmp!=NULL)
	{
