...
/* To Take Over Fucking SCRNSAVE.LIB, I need a dummy BiWindowFunc */
...
/*	FILE *fp;
    ...
    case WM_KEYDOWN:
    case WM_SYSKEYDOWN:
    	SendMessage(BiParentWnd,msg,wPara,lPara);
    	break;
	}
...
/* -> Take Over Fucking SCRNSAVE.LIB */
	...
	wCls.lpszClassName="YS_SCRNSAVE_CLASS";
	...
	module=GetModuleHandle(NULL);
	BiWnd=CreateWindow
	    ("YS_SCRNSAVE_CLASS","YS SCRNSAVE",WS_POPUP|WS_VISIBLE|WS_CLIPSIBLINGS|WS_CLIPCHILDREN,
	     lupX,lupY,sizX,sizY,NULL,NULL,module,NULL);
...
/*	 <- Take Over Fucking SCRNSAVE.LIB */
	...
	SetWindowPos(w,HWND_BOTTOM,0,0,1,1,SWP_NOREDRAW);
	//	MoveWindow(w,lupX,lupY,1,1,FALSE);  2002/05/12  MoveWindow must not be called.
	//                                                  Since w is a child window, MoveWindow
	//                                                  moves the window in the parent window's coordinate,
	//                                                  and yields an incorrect result.
	SetWindowPos(BiWnd,HWND_TOPMOST,lupX,lupY,sizX,sizY,SWP_NOREDRAW);
	MoveWindow(BiWnd,lupX,lupY,sizX,sizY,FALSE);
